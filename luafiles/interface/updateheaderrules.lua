...
local filepath = "/usr/local/acccontrol/files/header_rules_config"
...
-- update_data should be an array, each element is {header_name, rule_type, match_type, value, action}
...
for i, rule in ipairs(update_data) do
    if not rule.header_name or rule.header_name == "" then
        ngx.say('{"msg":"Rule ' .. i .. ' missing or empty header_name field"}')
        return ngx.exit(400)
    end
...
-- Update shared dict
header_rules_dict:set("header_rules", json_data)
