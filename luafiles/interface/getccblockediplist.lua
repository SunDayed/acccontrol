-- Get anti-scan banned IP list (with remarks)
...
local keys = cc_control_iplist:get_keys()
local now_ts = os.time()

for _, key in ipairs(keys) do
    if not seen[key] then
        local val = cc_control_iplist:get(key)
        local ttl_remaining = cc_control_iplist:ttl(key) -- Remaining seconds, 0 when no TTL
        local item = {
            ip = key,
...
local file, err = io.open("/usr/local/acccontrol/files/iplist_cc_control", "r")
if file then
