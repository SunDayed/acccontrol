-- Update rate-limit banned IP list (add/delete)
local cjson = require "cjson"

local access_number_iplist = ngx.shared.access_number_iplist
local file_path = "/usr/local/acccontrol/files/iplist_access_number"
...
if action == "add" then
    ...
    for _, ip in ipairs(ips) do
        ...
        access_number_iplist:set(ip, 1) -- value=1 manual perm ban
    end
    file:close()
...
elseif action == "delete" then
    ...
    for _, ip in ipairs(ips) do
        delete_set[ip] = true
        access_number_iplist:delete(ip)
    end
